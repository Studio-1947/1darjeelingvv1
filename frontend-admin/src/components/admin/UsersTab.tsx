...
/**
 * UsersTab displays all registered system users (Travelers, Providers, Admins)
 * and lists provider approval and status toggle controls.
 */
  ...
  return (
    <div className="mist-panel overflow-hidden border border-[var(--line)]">
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-mist text-ink-soft text-xs uppercase font-bold tracking-wider border-b border-[var(--line)]">
              ...
              <th className="p-4">Business / Status</th>
              ...
              <tr key={usr.id} className="hover:bg-mist/40 transition-colors">
                ...
                <td className="p-4 capitalize">
                  <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${
                    usr.role === 'admin' ? 'bg-flag/10 text-flag' : usr.role === 'provider' ? 'bg-pine/10 text-pine' : 'bg-mist text-ink-soft'
                  }`}>
                    {usr.role}
                  </span>
                </td>
                ...
                <td className="p-4">
                  {usr.role === 'provider' ? (
                    <div>
                      <div className="font-semibold text-xs text-ink">{usr.businessName || 'No Business Profile'}</div>
                      {usr.providerId && (
                ...
                <td className="p-4">
                  <div className="flex items-center justify-center gap-2">
                    ...
                    {usr.role === 'provider' && usr.providerId && (
                      <button
        ...
        {users.length === 0 && (
          <div className="p-8 text-center text-ink-soft">No registered users found.</div>
        )}
      </div>
    </div>
  );
