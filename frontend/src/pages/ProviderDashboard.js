...
export default function ProviderDashboard() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 md:px-8 py-8 md:py-10">
      <h1 className="font-display font-extrabold text-3xl sm:text-4xl text-ink">{t('provider.dashboard_title')}</h1>
      <div className="mt-6 md:mt-8 grid sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
        <div className="mist-panel p-5 md:p-6">
          ...
          <div className={`mt-2 flex items-center gap-2 font-display font-extrabold text-xl md:text-2xl ${active ? 'text-pine' : 'text-flag'}`} data-testid="provider-status">
        ...
        <div className="mist-panel p-5 md:p-6">
          ...
          <div className="mt-2 font-display font-extrabold text-lg md:text-xl text-ink">{provider.business_name}</div>
          <div className="text-sm text-ink-soft capitalize">{provider.business_type} · {provider.location}</div>
        </div>
        <div className="mist-panel p-5 md:p-6 sm:col-span-2 md:col-span-1">
          <div className="text-xs uppercase tracking-widest text-ink-soft">Contact</div>
          <div className="mt-2 font-display font-extrabold text-lg md:text-xl text-ink">{provider.contact_phone}</div>
        </div>
      </div>
      <div className="mt-6 md:mt-8 mist-panel p-5 md:p-6">
        <h3 className="font-display font-bold text-lg">Description</h3>
        <p className="text-sm md:text-base text-ink-soft mt-2 leading-relaxed">{provider.description}</p>
      </div>
    </div>
  );
}
