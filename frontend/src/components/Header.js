    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur border-b border-[var(--line)]" data-testid="site-header">
      <div className="mx-auto max-w-7xl px-5 md:px-8 h-16 flex items-center gap-4">
        <Link to="/" className="flex items-center gap-2" data-testid="brand-link">
          <div className="w-9 h-9 rounded-xl bg-pine text-white grid place-items-center font-display font-extrabold text-lg">১</div>
          <div className="leading-tight">
            <div className="font-display font-extrabold text-lg text-ink">{t('brand')}</div>
            <div className="text-[10px] text-ink-soft uppercase tracking-widest">Darjeeling</div>
          </div>
        </Link>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {user ? (
            <>
              ...
              <button onClick={() => { logout(); nav('/'); }} data-testid="logout-btn" className="inline-flex items-center gap-2 px-3 py-2 rounded-full border border-[var(--line)] text-ink font-semibold text-sm btn-hover">
            ...
            <>
              ...
              <Link to="/login" data-testid="header-login" className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-pine text-white font-semibold text-sm btn-hover">
